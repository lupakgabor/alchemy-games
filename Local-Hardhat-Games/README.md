...
Use the address you deployed your contract from step 3. Paste it into the `scripts/win_game{n}.js` file where the comments direct you to. 
...
Now run `npx hardhat run scripts/win_game{n}.js`, this will go ahead and call `win` on your `Game1` contract. If you're successful you should see a transaction receipt with a `Winner` event inside of the `events` array. 
...
3. Copy the address into the `scripts/win_game{n}.js`
4. Change the `contractName` in `scripts/win_game{n}.js`
